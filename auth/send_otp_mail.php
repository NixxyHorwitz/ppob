...
try {
    ...
    */
    ...
    if (!$updateOtp->execute([$otp, $user['id']])) {
...
border:3px solid #0056b3;padding:30px;border-radius:12px;color:#333;'>
    ...
    <p>
        Kami menerima permintaan untuk melakukan proses login ke akun Anda di 
        <strong>{$site_name}</strong>. Untuk melanjutkan proses verifikasi keamanan,
        silakan gunakan kode One-Time Password (OTP) berikut:
    </p>

    <p style='font-size:18px; text-align:center; margin:25px 0;'>
        <strong style='font-size:28px;color:#000;letter-spacing:3px;'>{$otp}</strong>
    </p>

    <p>
        Kode OTP ini bersifat <strong>rahasia</strong> dan hanya berlaku selama 
        <strong>5 menit</strong>. Demi menjaga keamanan akun Anda, 
        jangan pernah membagikan kode ini kepada siapapun, termasuk pihak yang 
        mengatasnamakan {$site_name}.
    </p>

    <p>
        Jika Anda tidak merasa melakukan permintaan login ini, segera abaikan email
...
";
    ...
    */
    ...
    header("Location: ../auth/verifikasi_otp");
    exit;
