...
if ($action === 'save_general') {
    ...
    if (!$site_name) {
        ...
        $pdo->prepare("UPDATE website_settings SET
            ...
            WHERE id=1
        ")->execute([$site_name, $site_tagline, $site_description,
                     $site_keywords, $site_author, $phone]);
...
if ($action === 'save_logo') {
    ...
    foreach (['site_logo' => 'logo', 'site_favicon' => 'favicon', 'seo_og_image' => 'og_image'] as $col => $field) {
        if (!empty($_FILES[$field]['tmp_name']) && $_FILES[$field]['error'] === UPLOAD_ERR_OK) {
            ...
            if (!$ok)          { $toast_e = "Format file $field tidak didukung."; break; }
            if ($size > 2097152) { $toast_e = "File $field terlalu besar (maks 2MB)."; break; }
...
if ($action === 'save_profile') {
    ...
    if (!$fullname) {
        ...
        if ($email) {
            ...
            if ($chk->fetch()) { $toast_e = 'Email sudah dipakai user lain.'; }
        }
        if (!$toast_e) {
...
<style>
  .settings-label{font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:.5px;color:var(--sub);margin-bottom:6px;}
  .settings-input{background:var(--hover)!important;border:1px solid var(--border)!important;color:var(--text)!important;border-radius:var(--rs)!important;font-size:13.5px;font-family:'Plus Jakarta Sans',sans-serif;transition:border-color .2s;}
  .settings-input:focus{border-color:var(--accent)!important;box-shadow:0 0 0 3px var(--ag)!important;}
  .settings-input::placeholder{color:var(--mut);}
  .settings-input option{background:var(--card);}
  .settings-input-addon{background:var(--card)!important;border:1px solid var(--border)!important;color:var(--sub)!important;border-radius:var(--rs)!important;}
  .settings-switch{background-color:var(--mut)!important;border-color:var(--mut)!important;}
  .settings-switch:checked{background-color:var(--accent)!important;border-color:var(--accent)!important;}
  .tab-nav-btn{display:flex;align-items:center;gap:10px;text-align:left;width:100%;padding:10px 12px;border-radius:var(--rs);border:none;background:transparent;color:var(--sub);transition:all .2s;cursor:pointer;}
  .tab-nav-btn:hover{background:var(--hover);color:var(--text);}
  .tab-nav-btn.active{background:var(--as);color:var(--accent);}
  ...
  .theme-opt{border:2px solid var(--border);border-radius:var(--r);padding:14px;cursor:pointer;transition:all .2s;}
  .theme-opt.active,.theme-opt:hover{border-color:var(--accent);}
    ...
    <div class="settings-tab-content">
      ...
      <div class="settings-pane active" id="pane-general" role="tabpanel">
        ...
        <div class="card-c mb-4">
          <div class="ch"><div><p class="ct">Informasi Situs</p><p class="cs">Data utama website PPOB Anda</p></div></div>
        ...
        <!-- Logo & Favicon -->
        <div class="card-c mb-4">
          <div class="ch"><div><p class="ct">Logo, Favicon & OG Image</p><p class="cs">Branding visual situs</p></div></div>
      ...
      <div class="settings-pane" id="pane-api" role="tabpanel">
        ...
        <div class="card-c mb-4">
          <div class="ch">
            <div><p class="ct">API Digiflazz</p><p class="cs">Konfigurasi koneksi ke provider PPOB</p></div>
        ...
        <div class="card-c">
          <div class="ch">
            <div><p class="ct">Metode Pembayaran Aktif</p><p class="cs">Data dari tabel payment_method</p></div>
      ...
      <div class="settings-pane" id="pane-profile" role="tabpanel">
        ...
        <!-- Avatar -->
        <div class="card-c mb-4">
          <div class="ch"><div><p class="ct">Foto Profil</p></div></div>
          <div class="cb">
        ...
        <!-- Info Personal -->
        <div class="card-c mb-4">
          <div class="ch"><div><p class="ct">Informasi Personal</p><p class="cs">Data akun admin yang sedang login</p></div></div>
          <form method="POST" class="cb">
        ...
        <div class="card-c">
          <div class="ch"><div><p class="ct">API Key Akun</p><p class="cs">Gunakan untuk akses API publik</p></div></div>
          <div class="cb">
            <div class="input-group">
      ...
      <div class="settings-pane" id="pane-security" role="tabpanel">
        ...
        <div class="card-c mb-4">
          <div class="ch"><div><p class="ct">Ubah Password</p><p class="cs">Minimal 8 karakter, kombinasi huruf & angka</p></div></div>
          <form method="POST" class="cb">
        ...
        <div class="card-c">
          <div class="ch">
            <div><p class="ct">Sesi Login Saat Ini</p></div>
            <form method="POST">
