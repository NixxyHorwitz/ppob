  ...
  Sortable.create(el, {
    ...
    onEnd(evt) {
      ...
      if (evt.from !== evt.to && newCatId && itemId) {
        ...
        smPost({ action:'move_item', id:itemId, category_id:newCatId });
      }
      smDirty(true);
    }
  });
...
function smSaveOrder() {
  ...
  smPost({ action:'reorder', ids: JSON.stringify(ids) }).then(() => {
    smDirty(false);
...
function smToggle(btn, id) {
  smPost({ action:'toggle', id }).then(r => {
    if (!r.ok) return;
...
function smDeleteCat(id, name) {
  ...
  smPost({ action:'delete', id }).then(r => {
...
function smSaveCat() {
  ...
  smPost({ action:'save_cat', id, cat_name, cat_slug, sort_order, ...(is_active ? {is_active:1} : {}) })
    .then(r => {
      btn.disabled = false;
...
function smUpdateIconHint(itype) {
  ...
  document.getElementById('smItemIconVal').placeholder = {ph:'ph-star', fa:'fas fa-home', img:'https://…'}[itype] || '';
}
...
function smSaveItem() {
  ...
  smPost({ action:'save_item', id, category_id, name, icon_type, icon_value, icon_bg, icon_color, href, query_cat, badge, badge_color, sort_order, ...(is_active?{is_active:1}:{}) })
    .then(r => {
      btn.disabled = false;
      if (!r.ok) return smToast(r.msg || 'Gagal menyimpan.', true);
      smModalItem.hide();
...
function smPost(data) {
  ...
  return fetch(SM_URL, { method:'POST', body:fd }).then(r => r.json()).catch(() => ({ok:false}));
}
