...
<style>
:root { --sky-blue: #0ea5e9; --sky-dark: #0284c7; }
body { background-color: #f8fafc; }

.card-custom { border: none; border-radius: 20px; margin-top: -30px; box-shadow: 0 10px 25px rgba(0,0,0,0.05); }


.nav-kategori { display: flex; gap: 10px; overflow-x: auto; padding: 10px 0; margin-bottom: 20px; scrollbar-width: none; }
.nav-kategori { 
    display: flex; 
    flex-wrap: wrap; 
    gap: 10px; 
...
.tab-item.active { background: var(--sky-blue); color: white; border-color: var(--sky-blue); }

.product-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(140px, 1fr)); gap: 12px; max-height: 450px; overflow-y: auto; padding: 10px; }
.product-card { border: 2px solid #e2e8f0; border-radius: 12px; padding: 15px; cursor: pointer; background: #fff; text-align: center; transition: 0.2s; }
.product-card:hover { border-color: var(--sky-blue); }
.product-card.active { border-color: var(--sky-blue); background: #f0f9ff; }
.product-name { font-size: 13px; font-weight: 600; color: #333; display: block; height: 40px; overflow: hidden; }
.product-price { font-size: 14px; font-weight: 700; color: var(--sky-dark); display: block; margin-top: 5px; }
.btn-navy { background-color: #134d74; color: white; border: none; border-radius: 15px; font-weight: 700; }
.btn-navy:hover {
    background-color: #1e5f8a; 
    color: #0ea5e9;
}
.main-content { margin-left: 260px; transition: all 0.3s ease; }
@media (max-width: 768px) { .main-content { margin-left: 0; } .product-grid { grid-template-columns: 1fr 1fr; } }
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('btnFinalBayar').addEventListener('click', function() {
        ...
        const hPin = document.createElement('input'); hPin.type = 'hidden'; hPin.name = 'pin_transaksi'; hPin.value = pin;
        const hBeli = document.createElement('input'); hBeli.type = 'hidden'; hBeli.name = 'beli'; hBeli.value = '1';
