...
<div class="hero">
    ...
    <div class="hero-in">
        ...
        <?php if (!empty($heroBanners)):
            ...
            /**
             * _hbStyle — bangun inline style dari kolom DB
             *
             * @param array  $hb     — row hero_banner
             * @param string $prefix — prefix kolom: 'strip_', 'img_left_', 'center_', dll
             * @param array  $extra  — style tambahan hardcoded (key => value CSS)
             *
             * Properti yang dibaca dari DB per prefix:
             *   top, right, bottom, left  → position (hanya aktif jika ada nilainya,
             *                               elemen otomatis position:relative jika terisi)
             *   pt, pr, pb, pl            → padding-top/right/bottom/left
             *   mt, mr, mb, ml            → margin-top/right/bottom/left
             *
             * Nilai kosong/NULL → properti di-skip (tidak di-override)
             */
            function _hbStyle(array $hb, string $prefix, array $extra = []): string
            {
                $map = [
                    'top'    => 'top',
                    'right'  => 'right',
                ...
                foreach ($map as $col => $css) {
                    $key = $prefix . $col;
                    $val = trim((string)($hb[$key] ?? ''));
                    if ($val === '') continue;
                    // sanitize — hanya izinkan karakter CSS yang aman
                    $val = preg_replace('/[^a-zA-Z0-9.\-% ]/', '', $val);
                    if ($val === '') continue;
                    $parts[] = "$css:$val";
                    if (in_array($col, ['top', 'right', 'bottom', 'left'])) $hasPos = true;
                }

                // Jika ada top/right/bottom/left → tambah position:relative otomatis
                if ($hasPos) array_unshift($parts, 'position:relative');

                // Merge extra hardcoded styles
            ...
            foreach ($heroBanners as $hb):
                ...
                // Style per elemen
                $sStrip      = _hbStyle($hb, 'strip_');
                $sSideLeft   = _hbStyle($hb, 'img_left_',   ['width' => (int)($hb['img_left_width'] ?? 90) . 'px', 'height' => $h . 'px']);
                $sCenter     = _hbStyle($hb, 'center_',     ['min-height' => $h . 'px']);
                $sTitle      = _hbStyle($hb, 'title_',      ['color' => htmlspecialchars($hb['title_color'] ?? '#fff')]);
                $sSub        = _hbStyle($hb, 'sub_',        ['color' => htmlspecialchars($hb['subtitle_color'] ?? '#fff')]);
                $sCenterImg  = _hbStyle($hb, 'center_img_', ['width' => (int)($hb['center_image_width'] ?? 160) . 'px', 'max-height' => ($h - 20) . 'px', 'object-fit' => 'contain']);
                $sBtn        = _hbStyle($hb, 'btn_',        ['background' => htmlspecialchars($hb['btn_color'] ?? '#FFD700'), 'color' => htmlspecialchars($hb['btn_text_color'] ?? '#000')]);
                $sSideRight  = _hbStyle($hb, 'img_right_',  ['width' => (int)($hb['img_right_width'] ?? 90) . 'px', 'height' => $h . 'px']);
        ?>
                <div class="hd-strip" <?= $sStrip ? " style=\"$sStrip\"" : '' ?>>

                    <!-- Gambar Kiri -->
                    <?php if (!empty($hb['img_left'])): ?>
                        <div class="hd-side" <?= $sSideLeft ? " style=\"$sSideLeft\"" : '' ?>>
                    ...
                    <!-- Tengah -->
                    <div class="hd-center" <?= $sCenter ? " style=\"$sCenter\"" : '' ?>>
                        <?php if (($hb['center_type'] ?? 'text') === 'image' && !empty($hb['center_image'])): ?>
                            <img src="<?= htmlspecialchars($hb['center_image']) ?>"
                                ...
                                <?= $sCenterImg ? "style=\"$sCenterImg\"" : '' ?>
                                alt="">
                        <?php else: ?>
                            <?php if (!empty($hb['title'])): ?>
                                <div class="hd-title" <?= $sTitle ? " style=\"$sTitle\"" : '' ?>>
                                    <?= htmlspecialchars($hb['title']) ?>
                                </div>
                            <?php endif; ?>
                            <?php if (!empty($hb['subtitle'])): ?>
                                <div class="hd-sub" <?= $sSub ? " style=\"$sSub\"" : '' ?>>
                    ...
                    <?php if (!empty($hb['img_right'])): ?>
                        <div class="hd-side" <?= $sSideRight ? " style=\"$sSideRight\"" : '' ?>>
